...
classdef asdkDM < handle
    ...
    methods
        ...
        function SendPattern( obj, values, nRepeat )
            ...
            assert( isvector(values) && mod(numel( values ),obj.nAct) == 0, ...
        ...
        function value = GetVector( obj, cmdName )
            %% Get vector from SDK
            % Parameters:
            %   cmdName - name of the value.
            
            value = asdkMex( 50, obj.dmId, cmdName );
        end

        function value = GetString( obj, cmdName )
            %% Get string from SDK
            % Parameters:
            %   cmdName - name of the value.
            
            value = asdkMex( 51, obj.dmId, cmdName );
        end

        function SetVector( obj, cmdName, value )
            %% Set value from SDK
            % Parameters:
            %   cmdName - name of the value. 
            %   value   - value to set (vector)
            
            asdkMex( 60, obj.dmId, cmdName, value, numel(value) );
        end
    ...
    methods (Hidden)
        ...
        function ptr = GetDmObjectPointer(obj)
            %% Get the pointer to DM object to be used in the same address space
            % as uint64_t

            ptr = asdkMex( 9, obj.dmId );
        end
    end
